...
const REVERSE_ID_TABLE = Object.keys(ID_TABLE).reduce((p, c) => {
  p[ID_TABLE[c]] = c;
  return p;
}, {});
...
const createBundleLookup = Object.keys(Bundles.content).reduce((p, key) => {
  const [roomName, bundleId] = key.split('/');
  const [bundleName, rawReward, ingredients, ..._] = Bundles.content[key].split(
    '/'
  );

  const splittedReward = rawReward.split(' ');

  const reward = {
    id: +splittedReward[1],
    name:
      splittedReward[0] === 'BO'
        ? REWARD_NAME_TABLE[splittedReward[1]]
        : REVERSE_ID_TABLE[splittedReward[1]],
    stack: splittedReward[2],
  };

  const splittedIngredients = ingredients.split(' ');
  const allIngredients = [];
  for (let i = 0; i < splittedIngredients.length; i += 3) {
    const itemId = +splittedIngredients[i];
    const stack = +splittedIngredients[i + 1];
    const quality = +splittedIngredients[i + 2];
    allIngredients.push({
      itemId,
      stack,
      quality,
      name: REVERSE_ID_TABLE[itemId],
    });
  }
...
console.log(JSON.stringify(createBundleLookup));
